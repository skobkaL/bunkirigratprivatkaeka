...
function playerRow(p, st, opts = {}) {
  ...
  return `<li class="player-list" style="list-style:none"><div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
  ...
  if (st.phase === 'voting') {
    if (meAlive) {
      ...
      html += st.players.filter(p => p.alive && p.id !== st.meId).map(p =>
        `<div class="vote-card ${myVote === p.id ? 'selected' : ''}" data-vote="${p.id}">${esc(p.name)}</div>`).join('');
      html += `</div><div class="row"><button id="btn-confirm-vote" class="btn btn-primary" ${myVote ? '' : 'disabled'}>Подтвердить голос</button></div>`;
      box.innerHTML = html;
      document.querySelectorAll('[data-vote]').forEach(el => el.onclick = () => { selectVote(el.dataset.vote); });
      $('btn-confirm-vote').onclick = () => socket.emit('vote', selectedVote, r => r?.error ? toast(r.error) : toast('Голос принят'));
    } else {
